...
describe('Dex Test', () => {
  ...
  describe("Deployment", () => {
    ...
    it("should assign the total link supply of tokens to the owner", async () => {
  ...
  describe("Transactions", () => {
    it("should throw an error if ETH balance is insufficient when crating a BUY limit order", async () => {
      ...
      expect(
        dex.createLimitOrder(0, ethers.utils.formatBytes32String("LINK"), 10, 1)
      ).to.be.reverted
    ...
    it("should allow transactions for BUY limit orders when ETH balance is sufficient", async () => {
      ...
      const buyLimitOrderTx = await dex.createLimitOrder(0, ethers.utils.formatBytes32String("LINK"), 10, 1);
    ...
    it("should throw an error if token balance is insufficient when creating a SELL limit order", async () => {
      const dexTokenBalance = await dex.balances(owner.address, ethers.utils.formatBytes32String("LINK"));

      await expect(dexTokenBalance).to.equal(0);

      await expect(
        dex.createLimitOrder(1, ethers.utils.formatBytes32String("LINK"), 10, 1)
      ).to.be.reverted;
      ...
      const depositTx = await dex.deposit(10, ethers.utils.formatBytes32String("LINK"));
      await depositTx.wait();

      const limitOrderTx = await dex.createLimitOrder(1, ethers.utils.formatBytes32String("LINK"), 10, 1);
    ...
    it("The Limit BUY order book should be ordered on price from highest to lowest starting at index 0", async () => {
      ...
      await link.approve(dex.address, 500); // 500 Link tokens approved
      await dex.depositEth({ value: 3000 }); // in Wei

      await dex.createLimitOrder(0, ethers.utils.formatBytes32String("LINK"), 1, 300);
      await dex.createLimitOrder(0, ethers.utils.formatBytes32String("LINK"), 1, 100);
      await dex.createLimitOrder(0, ethers.utils.formatBytes32String("LINK"), 1, 200);

      // get BUY side
      const orderBook = await dex.getOrderBook(ethers.utils.formatBytes32String("LINK"), 0); 
    ...
    it("The SELL order book should be ordered on price from lowest to highest starting at index 0", async () => {
      ...
      await dex.createLimitOrder(1, ethers.utils.formatBytes32String("LINK"), 1, 300);
      await dex.createLimitOrder(1, ethers.utils.formatBytes32String("LINK"), 1, 100);
      await dex.createLimitOrder(1, ethers.utils.formatBytes32String("LINK"), 1, 200);
